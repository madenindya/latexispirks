...
%-----------------------------------------------------------------------------%
Proses evaluasi \textit{pair} dilakukan hanya terhadap \textit{pair} yang diterima ke dalam korpus pasangan kata. Anotasi \textit{pair} dilakukan oleh tiga anotator berbeda dengan daftar \textit{pair} yang sama. Total sampel yang dianotasi adalah 514 \textit{pair}, dimana sampel tersebut diambil dari gabungan ketiga percobaan.
...
Dari data di atas, dapat dihitung akurasi kebenaran sampel. Untuk data yang memiliki label berbeda seperti kolum 
`2 Benar' dan `1 Benar', diambil nilai kebenarannya menggunakan sistem \textit{voting}. Akurasi total yang diadapatkan dari seluruh data sampel adalah 0.83. Dari sisi akurasi, korpus yang dihasilkan dari seluruh eksperimen dapat dikatakan cukup baik.

Untuk mengevaluasi parameter \textit{semi-supervised} yang dimodifikasi, perlu dicari dihitung akurasi untuk setiap eksperimen sehingga dapat dibandingkan. Tabel \ref{table:akurasi-all} memperlihatkan akurasi \textit{pair} untuk setiap eksperimen.
