...
Sebagai contoh, terdapat pasangan relasi kata `(sepak bola;olahraga)' serta kalimat `\textit{{\fontfamily{qcr}\selectfont<start>} sepak bola adalah cabang olahraga yang menggunakan bola yang umumnya terbuat dari bahan kulit dan dimainkan oleh dua tim . \textit{{\fontfamily{qcr}\selectfont<end>}'. Korpus kalimat dengan tag hipernim-hiponim yang dihasilkan adalah `\textit{{\fontfamily{qcr}\selectfont<start>} {\fontfamily{qcr}\selectfont<hyponym>}sepak bola{\fontfamily{qcr}\selectfont<hyponym>} adalah cabang {\fontfamily{qcr}\selectfont<hypernym>}olahraga{\fontfamily{qcr}\selectfont<hypernym>} yang menggunakan bola yang umumnya terbuat dari bahan kulit dan dimainkan oleh dua tim . \textit{{\fontfamily{qcr}\selectfont<end>}'. Dengan adanya penanda kata mana yang merupakan hipernim dan hiponim, proses ekstraksi \textit{pattern} akan lebih mudah dilaksanakan.
...
\begin{itemize}
  \item \textit{{\fontfamily{qcr}\selectfont<start>} {\fontfamily{qcr}\selectfont<hyponym>}kelinci{\fontfamily{qcr}\selectfont<hyponym>} adalah {\fontfamily{qcr}\selectfont<hypernym>}binatang{\fontfamily{qcr}\selectfont<hypernym>} yang gemar memakan wortel . \textit{{\fontfamily{qcr}\selectfont<end>}
  \item \textit{{\fontfamily{qcr}\selectfont<start>} {\fontfamily{qcr}\selectfont<hyponym>}mobil{\fontfamily{qcr}\selectfont<hyponym>} adalah {\fontfamily{qcr}\selectfont<hypernym>}kendaaraan{\fontfamily{qcr}\selectfont<hypernym>} beroda empat . \textit{{\fontfamily{qcr}\selectfont<end>}
  \item \textit{{\fontfamily{qcr}\selectfont<start>} selain sepak bola, {\fontfamily{qcr}\selectfont<hyponym>}basket{\fontfamily{qcr}\selectfont<hyponym>} adalah {\fontfamily{qcr}\selectfont<hypernym>}olahraga{\fontfamily{qcr}\selectfont<hypernym>} yang disukai masyarakat . \textit{{\fontfamily{qcr}\selectfont<end>}
\end{itemize}

\noindent Dari kalimat di atas \textit{pattern} unik yang dihasilkan yaitu `{\fontfamily{qcr}\selectfont<hyponym>} adalah {\fontfamily{qcr}\selectfont<hypernym>}'. Lebih detil tahapan pembentukan \textit{pattern} dapat dilihat pada bab 4.4.
...
Sebagai contoh, terdapat \textit{pattern} leksikal `{\fontfamily{qcr}\selectfont<hyponym>} adalah {\fontfamily{qcr}\selectfont<hypernym>} yang'. Berikut adalah beberapa kalimat Wikipedia dengan \textit{postag}.

\begin{itemize}
  \item \textit{{\fontfamily{qcr}\selectfont<start>}\_X sepak\_NNP bola\_NNP adalah\_VB olahraga\_NN paling\_RB populer\_JJ di\_IN Britania\_NNP .\_Z \textit{{\fontfamily{qcr}\selectfont<end>}\_X
  \item \textit{{\fontfamily{qcr}\selectfont<start>}\_X Berikut\_VB adalah\_VB daftar\_NN penghargaan\_NN yang\_SC diberikan\_VB .\_Z \textit{{\fontfamily{qcr}\selectfont<end>}\_X
  \item \textit{{\fontfamily{qcr}\selectfont<start>}\_X Gado-gado\_NNP adalah\_VB makanan\_NN yang\_SC berasal\_VB dari\_IN Betawi\_NN .\_Z \textit{{\fontfamily{qcr}\selectfont<end>}\_X
\end{itemize}
